...
var llenarViewInfraccion, viewOnlyObservaciones;

viewOnlyObservaciones = function() {
  var observaciones;
  if (window.view_infraccion) {
    observaciones = window.view_infraccion.observaciones;
    return $('#view_observaciones_inf').html(observaciones);
  }
};

window.viewOnlyCertificadoMedico = function() {
  var certificado;
  if (window.view_infraccion) {
...
window.onlyViewRecoje = function() {
  var ocultarRecojeLicencia, ocultarRecojeVehiculo, se_recoje, showRecojeLicencia, showRecojeVehiculo;
...
window.onlyViewArticulos = function() {
  var articulo, articulos, row, tabla, _i, _len, _results;
  if (window.view_infraccion) {
    tabla = $("#view_tab_articulos_spa_inf");
    $("#view_tab_articulos_spa_inf>tbody").empty();
    articulos = window.view_infraccion.articulos;
    _results = [];
    for (_i = 0, _len = articulos.length; _i < _len; _i++) {
      articulo = articulos[_i];
      if (articulo || articulo !== void 0) {
        ...
        _results.push(tabla.append(row));
      } else {
...
window.onlyViewCaracteristicasVehiculo = function() {
  var vehiculo;
  if (window.view_infraccion) {
    ...
    return $('#view_no_tarjeton_cv_spa_inf').html(vehiculo.no_tarjeton);
  }
};

window.onlyViewInfractor = function() {
  var infractor;
  if (window.view_infraccion) {
    ...
    return $('#view_no_licencia_di_sp_inf').html(infractor.no_licencia);
  }
};
...
llenarViewInfraccion = function() {
  ...
  if (window.view_infraccion) {
    ...
    */
    ...
    return $('#view_circulacion_spa_inf').html(inf.circulacion);
    /*
    		if inf.firmo== 'si'
    			$('#si_firmo_inf').click()
    		else 
